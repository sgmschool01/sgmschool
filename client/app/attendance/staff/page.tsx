    ...
    useEffect(() => {
        const fetchMeta = async () => {
            try {
                ...
                const yearsList: AcademicYear[] = Array.isArray(yearsRes)
                    ? yearsRes
                    : (Array.isArray(yearsRes?.years) ? yearsRes.years : []);
                if (yearsList.length > 0) {
                    setAcademicYears(yearsList);
                    const active = yearsRes?.active_year || yearsList.find((y: AcademicYear) => y.is_active || y.status === 'active') || yearsList[0];
                    if (active) {
